...
for (let i = 1; i <= 23; i++) {
    ...
    const oldShowQuestion = `        function showQuestion(index) {
            ...
            document.getElementById('question-container').innerHTML = \`
                <div class="question">
                    ...
                    \${q.options.map(opt => \`<label class="option" id="opt-\${opt.letter}"><input type="\${inputType}" name="answer" value="\${opt.letter}" \${userAnswers[index]?.includes(opt.letter) ? 'checked' : ''} onchange="selectOption('\${opt.letter}', this.checked)"><span><strong>\${opt.letter}.</strong> \${opt.text}</span></label>\`).join('')}
                </div>\`;
    ...
    const newShowQuestion = `        function showQuestion(index, showAnswer = false) {
            currentQuestion = index;
            const q = questions[index], isMultiple = q.correct.length > 1, inputType = isMultiple ? 'checkbox' : 'radio';
    ...
    const oldNav = `        function goToQuestion(index) { showQuestion(index); }
